...
**概要**: 過去問一覧ページ + 年度別詳細ページ + PDF ダウンロード
...
- 各年度のデータを JSON または Markdown で管理
- PDF ファイルは`/public/past-exams/`に配置
...
- **ファイルサイズ**: 圧縮を検討
- **ストレージ**: `/public`フォルダまたは CDN
- **ライセンス**: 公式 PDF の利用規約を確認
...
- ✅ **公式サイトへのリンクを提供**
  - 最も安全な方法
  - 利用規約違反のリスクが低い
- ✅ **公式 PDF を iframe で埋め込み（公式 URL を直接参照）**
  - PDF を自サイトに保存しない
  - 公式サイトの URL を直接参照
- ✅ **公式サイトの利用規約を確認してから実装**
...
#### 方法 A: 公式サイトへのリンク集（最も安全）⭐

```tsx
// 公式サイトへのリンクのみを提供
<a
  href="https://www.fsa.go.jp/cpaaob/kouninkaikeishi-shiken/..."
  target="_blank"
...
#### 方法 B: 公式 PDF を iframe で埋め込み（公式 URL 参照）
...
- ⚠️ 公式サイトの URL が変更される可能性

---

#### 方法 C: 公式サイトの利用規約確認後の実装
...
### Phase 2: 公式 PDF の iframe 埋め込み（利用規約確認後）
...
PDF を自サイトに保存する方法は、**利用規約で明示的に許可されている場合のみ**実装してください。
